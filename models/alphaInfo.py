...
# 定义映射类
class AlphaInfo(Base):
    __tablename__ = 't_alpha_info'

    # 主键字段
    f_id = Column(Integer, primary_key=True, autoincrement=True, comment='主键 ID')

    # 其他字段
    f_name = Column(String(255), nullable=False, default='', comment='公司名称')
    f_symbol = Column(String(20), nullable=False, default='', comment='Alpha Vantage API 参数')
    f_symbol_to = Column(String(20), nullable=False, default='', comment='外汇参数')
    ...
    def __repr__(self):
        return f"<AlphaInfo(f_id={self.f_id}, f_name='{self.f_name}', f_symbol='{self.f_symbol}', f_symbol_to='{self.f_symbol_to}')>"
